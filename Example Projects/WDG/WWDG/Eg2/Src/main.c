/******************************************************************//**
* @file   main.c
* @brief  Write a program to understand watch dog timer using Early wakeup interrupt.
* @version  v1.0
* @date   Apr 16, 2015
...
int main(void)
{
  ...
  BSP_LED_Init(LED5);                          // initialize on baord LED 3 and LED 5
  BSP_LED_Init(LED6);
  ...
  if(__HAL_RCC_GET_FLAG(RCC_FLAG_WWDGRST) != RESET)
  {
    ...
    BSP_LED_On(LED5);
    trace_printf("system has resumed from WWDG reset\n\r");
    /* Clear reset flags */
   ...
   */
  BSP_WWDG_Init(WWDG_PRESCALER_8,80,127); //initialize WWDG
  __HAL_WWDG_CLEAR_IT(&hwwdg_bsp,WWDG_FLAG_EWIF);
  HAL_WWDG_Start_IT(&hwwdg_bsp);
  /* USER CODE END 2 */
...
HAL_WWDG_WakeupCallback(WWDG_HandleTypeDef* hwwdg)
{
  ...
  {
    trace_printf("WWDG Reset occur\n\r");   //type your function here
  }
}
/* USER CODE END 4 */
