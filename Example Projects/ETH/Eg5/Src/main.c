/******************************************************************//**
* @file   main.c
* @brief  Write a Program to transmit raw packet via Ethernet.
* @version  v1.0
* @date   Apr 16, 2015
...
int main(void)
{
  ...
  uint8_t txpkt[118];
  uint8_t payload[100] = {"This is a Raw Packet"};
  struct pbuf_custom pc;
  __ALIGN_BEGIN ETH_DMADescTypeDef  DMATxDscr __ALIGN_END;/* Ethernet Tx DMA Descriptor */
  /* USER CODE END 1 */
  ...
  /* Create packet */
  PacketGen(txpkt,payload);

  /* send packet to trans buffer */
  pbuf_alloced_custom(3,118,PBUF_REF,&pc,&txpkt,118);

  /* Initialize ETH */
  BSP_ETH_Init(&heth);

  /* Initialize Tx Descriptors list: Chain Mode */
  if(HAL_ETH_DMATxDescListInit(&heth, &DMATxDscr, &txpkt[0], 1) != HAL_OK)
  {
    while(1);
  }
  ...
  /* USER CODE BEGIN 3 */
    /* Transmit packet */
    if (HAL_ETH_TransmitFrame(&heth,118) != HAL_OK)
    {
      while(1);
    }
    HAL_Delay(5000);
