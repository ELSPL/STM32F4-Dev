/******************************************************************//**
* @file   main.c
* @brief  Write a program to test Ethernet client(UDP protocol)
* @version  v1.0
* @date   Apr 16, 2015
...
int main(void)
{
  ...
  /* USER CODE BEGIN 3 */
    ...
    sys_check_timeouts();
    data = ugetche(NONE_BLOCKING);
    switch (data)
    {
      case '1':
        ethernetif_set_link(&gnetif);
        uprintf("Ethernet interface Link set\n\r");
        break;

      case '2':
        udp_echoclient_send();
        uprintf("Connected to UDP echo server\n\r");
        break;
