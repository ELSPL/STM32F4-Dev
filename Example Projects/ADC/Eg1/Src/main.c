/******************************************************************//**
* @file   main.c
* @brief  Write a program to use ADC conversion in polling mode.
* @version  v1.0
* @date   Apr 16, 2015
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  BSP_ADC_Init(&hadc_bsp1,ADC_IN0,1,Indepenent_Mode);
  HAL_ADC_Start(&hadc_bsp1);   // Start ADC1
  ...
  /* USER CODE BEGIN 3 */
    HAL_ADC_PollForConversion(&hadc_bsp1, 5000);  // check the port pin for max 5 sec

    /* Check if the continuous conversion of regular channel is finished */
    if(HAL_ADC_GetState(&hadc_bsp1) == HAL_ADC_STATE_EOC_REG)
    {
      ConvertedValue = HAL_ADC_GetValue(&hadc_bsp1);
      trace_printf("%04d\n\r",ConvertedValue);
      HAL_Delay(500);
    }
