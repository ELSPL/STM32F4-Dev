/******************************************************************//**
* @file   main.c
* @brief  Write a program to Use ADC1, ADC2 and ADC3 in Triple interleaved mode and DMA mode2.
* @version  v1.0
* @date   Apr 16, 2015
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  /* Initialize ADC */
  BSP_ADC_Init(&hadc_bsp1,ADC_IN0,1,Triple_Mode_INTERL);
  BSP_ADC_Init(&hadc_bsp2,ADC_IN0,1,Triple_Mode_INTERL);
  BSP_ADC_Init(&hadc_bsp3,ADC_IN0,1,Triple_Mode_INTERL);
  HAL_ADC_Start(&hadc_bsp3);                                    // Start ADC3
  HAL_ADC_Start(&hadc_bsp2);                                    // Start ADC2
  HAL_ADCEx_MultiModeStart_DMA(&hadc_bsp1, ConvertedValue,3);   // Start ADC1 with DMA
  /* USER CODE END 2 */
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ConvertedValue[i] = HAL_ADCEx_MultiModeGetValue(&hadc_bsp1);
  }
  trace_printf("%08x\n\r",ConvertedValue[0]);
  trace_printf("%08x\n\r",ConvertedValue[1]);
  trace_printf("%08x\n\r",ConvertedValue[2]);
