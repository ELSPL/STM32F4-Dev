 ...
 */
static void handleRxWiFi(uint8_t* buf, int16_t len)
{
  ...
  switch (subCmdId)
  {
    case WIFI_GET_STATUS_RSP:
      ...
      else
      {
        ...
        val[strlen(val)-1] = 0;
        uprintf("WiFi On.  Mac: %s.  ", val);
        if (buf[2] == MODE_NO_NETWORK)
        {
          uprintf("Not joined any network.\n\r");
        }
        else
        {
          uprintf("Joined SSID: %s\n\r", buf+9);
        }
      }
      break;
 ...
 * @param len length of data
 */
static void handleRxSNIC(uint8_t* buf, int32_t len)
{
  uint8_t subCmdId = buf[0];
  ...
  switch (subCmdId)
  {
    ...
    case SNIC_GET_DHCP_INFO_RSP:
      ...
      if (SNIC_SUCCESS == buf[2])
      {
        uprintf("IP assigned as %d03.%d03.%d03.%d03 \n\r", buf[9],buf[10],buf[11],buf[12]);
          //save IP
        memcpy(&selfIP, buf+9, 4);
      }
      else
      {
        uprintf("IP not assigned\n\r");
      }
      break;
    ...
    case SNIC_TCP_CONNECT_TO_SERVER_RSP:
      ...
      {
        uprintf("Unable to connect server\n\r");
      }
      break;
    ...
    case SNIC_TCP_CREATE_CONNECTION_RSP:
      if (SNIC_SUCCESS != buf[2])
      {
        uprintf("Unable to create TCP server\n\r");
      }
      break;
    ...
    case SNIC_TCP_CONNECTION_STATUS_IND:
      ...
      {
        uprintf("Socket connection UP\n\r");
        sockConnected = buf[3];
      }
      else if (SNIC_CONNECTION_CLOSED == buf[2])
      {
        uprintf("Socket %d02 closed\n\r", buf[3]);
        sockClosed = buf[3];
      }
      break;
    ...
    case SNIC_SEND_RSP:
    {
      ...
      if (SNIC_SUCCESS == buf[2])
      {
        ...
        uprintf("pkt %d03, %d03 bytes sent \n\r", pktcnt, sentsize);
      }
      break;
    }
    case SNIC_CONNECTION_RECV_IND:
    {
      ...
      uprintf("%d03 bytes received from socket %d02 \n\r", sentsize, sock);
      HAL_Delay(10);
      if (strncmp((int8_t*)buf+5, "GET /", 5) == 0 || strncmp((int8_t*)buf+5, "POST /", 6) == 0)  // Receives a HTTP(S) get/post request.
      {
        ...
        // Send back something
        SendFromSock(sock, (uint8_t*)out, strlen(out), 2, seqNo++);
      ...
      else if (strncmp((int8_t*)buf+5, "HTTP/1.", 7) == 0)  // Receives HTTP response, close socket. If the socket is not closed,
      {
        ...
        CloseSocket((uint8_t)sock,seqNo++); }
      break;
    }
    case SNIC_TCP_CLIENT_SOCKET_IND:
    {
       uint8_t listen_sock = buf[2];
       uprintf("Accepted connection from %d03.%d03.%d03.%d03\n\r", buf[4], buf[5], buf[6], buf[7]);
       uprintf("Connection socket: %d02\n\r", buf[3]);
       break;
    }
...
int8_t GetTCPInfo(void)
{
  ...
  uprintf("Enter server IP to connect: \n\r");
  uget_line(tempIPstr,sizeof(tempIPstr));
//    scanf("%s", tempIPstr);
  uprintf("\n\r");
  if (strlen(tempIPstr))
  {
  ...
  uprintf("Enter server port number: \n\r");
  uget_line(teststr,sizeof(teststr));
//    scanf("%s", teststr);
  uprintf("\n\r");
  if (strlen(teststr))
  {
...
int8_t SetTCPInfo(void)
{
  ...
  if (selfIP == 0)
  {
    uprintf("IP address has not been obtained.\n\r");
    return CMD_ERROR;
  }
  srcIP = selfIP;

  uprintf("Enter server port number to set: \n\r");
  uget_line(teststr,sizeof(teststr));
//  scanf("%s", teststr);
  uprintf("\n\r");
  ...
  if (srcPort > 0xFFFF)
  {
    uprintf("Invalid port, max limit 0xFFFF \n\r");
    return CMD_ERROR;
  }
  return 0;
}
...
//  scanf("%s", SSID);
  ...
  while(!strlen(SSID))
  {
    uprintf("SSID can't be empty. Enter SSID: \n\r");
    uget_line(SSID,sizeof(SSID));
//      scanf("%s", SSID);
    uprintf("\n\r");
  }
  ...
  p += strlen(SSID);
  *p++ = 0x00;
  uprintf("Enter Security Mode (e.g., 0 for open, 2 for WPA TKIP, 4 for WPA2 AES, 6 for WPA2 MIXED):\n\r");
...
//  scanf("%s", tempstr);
  ...
  if (secMode)
  {
    uprintf("Enter Security Key:\n\r");
    uget_line(secKey,sizeof(secKey));
//    scanf("%s", secKey);
    uprintf("\n\r");
...
{
  uprintf("----------------------\n\r");
  uprintf("0 Get WiFi status\n\r");
  uprintf("1 Wifi Scan\n\r");
  uprintf("2 Wifi Join\n\r");
  uprintf("3 Get IP\n\r");
  uprintf("4 TCP client\n\r");
  uprintf("5 TCP sever\n\r");
  uprintf("6 Send from sock\n\r");
  uprintf("7 WiFi Leave\n\r");
  uprintf("8 AP On/Off\n\r");
  uprintf("9 UDP client\n\r");
  uprintf("a UDP server\n\r");
  uprintf("b Wifi Off\n\r");
  uprintf("c Wifi On\n\r");
...
int8_t SendHttpReqTest(int8_t *domain, int8_t isHttps)
{
  ...
  int8_t content[] = "";
  uint8_t timeout = 10;
  uprintf("Enter URI after the server name: ([CR] to accept %s)\n\r", uri);
...
//  scanf("%s",tmp);
  ...
  uprintf("Enter content to POST: ([CR] to accept %s)\n\r", content);
  uget_line(tmp, sizeof(tmp));
//  scanf("%s",tmp);
  uprintf("\n\r");
  if (strlen(tmp))
  ...
  switch(key)
  {
    ...
    case '4':
      ...
      if (mysock != -1)
      {
        ...
        TCPConnectToServer(mysock, destIP, (uint16_t)destPort, 0x0400, 0x5, seqNo++);
      }
      break;
    ...
    case '5':
      ...
      mysock = -1;
      TCPCreateSocket(1, srcIP, (uint16_t)srcPort, seqNo++, SNIC_TCP_CREATE_SOCKET_REQ);
      if (mysock != -1)
      {
    ...
    case '6':
    {
      ...
      uprintf("Enter socket number to send from: \n\r");
      uget_line(sockstr,sizeof(sockstr));
      uprintf("\n\r");
//      scanf("%s", sockstr);
      sock = strtol(sockstr, NULL, 0);
      uprintf("Content Option? (0: Default 128bytes will be sent) \n\r");
      uget_line(tempstr,sizeof(tempstr));
      uprintf("\n\r");
//      scanf("%s", tempstr);
