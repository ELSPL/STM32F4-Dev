...
uint8_t BSP_RTC_Change_Time (void)
{
  ...
  while(1)
  {
    cp = vgetche(BLOCKING);         /* get input character */
    ...
    else if (cp>= '0' && cp <='9')/* is character between 0 to 9? */
    {
      ...
      if(count < MaxCount)
      {
        if(State == 0)
        {
          if(digit == 0)
          {
            vuprintf("%c",RTCdata);     /* echo 1st character */
          ...
          {
            vuprintf("%c", RTCdata);               /* echo 2nd character */
