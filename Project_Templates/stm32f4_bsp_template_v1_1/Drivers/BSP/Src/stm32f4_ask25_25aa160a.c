 ...
 * @{
 */
SPI_HandleTypeDef hspi1_25aa160a;
...
/** @defgroup STM32F4_ASK25_25AA160A_Private_Functions
  ...
  */
/**
 * @brief This function is support function for ASK25_SPI1_Init
 *        This function configures GPIO of SPI1
 * @param hspi  Provide SPI handle
 *      @arg  &hspi1_25aa160a
 * @retval None
 */
static void ASK25_SPI1_MspInit(SPI_HandleTypeDef* hspi)
{
 ...
 * @return  None
 */
static void SPI1_CS_Force (FunctionalState state)
{
  if (state)
  {
...
/**
 * @brief This Function configures and Initializes SPI1
 * @retval None
 */
void ASK25_SPI1_Init(void)
{
  ...
  hspi1_25aa160a.Instance = SPI1;
  hspi1_25aa160a.Init.Mode = SPI_MODE_MASTER;
  hspi1_25aa160a.Init.Direction = SPI_DIRECTION_2LINES;
  hspi1_25aa160a.Init.DataSize = SPI_DATASIZE_8BIT;
  hspi1_25aa160a.Init.CLKPolarity = SPI_POLARITY_LOW;
  hspi1_25aa160a.Init.CLKPhase = SPI_PHASE_1EDGE;
  hspi1_25aa160a.Init.NSS = SPI_NSS_SOFT;
  hspi1_25aa160a.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_128;
  hspi1_25aa160a.Init.FirstBit = SPI_FIRSTBIT_MSB;
  hspi1_25aa160a.Init.TIMode = SPI_TIMODE_DISABLED;
  hspi1_25aa160a.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLED;

  ASK25_SPI1_MspInit(&hspi1_25aa160a);
  HAL_SPI_Init(&hspi1_25aa160a);
}


/**
 * @brief This function is for De-Initializing SPI1
 * @param hspi Provide SPI handle
 *      @arg  &hspi1_25aa160a
 * @retval None
 */
...
uint8_t ASK25_25AA160A_Read_Status_Reg (void)
{
  ...
  SPI1_CS_Force(DISABLE);
  HAL_SPI_Transmit(&hspi1_25aa160a, inst, 1,500);
  status = HAL_SPI_Receive(&hspi1_25aa160a, reg, 1,500);
  if(status == HAL_OK)
  {
    SPI1_CS_Force(ENABLE);
  }
...
HAL_StatusTypeDef ASK25_25AA160A_Write_Status_Reg (uint8_t status_reg)
{
  ...
  SPI1_CS_Force(DISABLE);
  status = HAL_SPI_Transmit(&hspi1_25aa160a, &WE, 1, 500);
  if(status == HAL_OK)
  {
    SPI1_CS_Force(ENABLE);
  }
  ...
  SPI1_CS_Force(DISABLE);
  status = HAL_SPI_Transmit(&hspi1_25aa160a, tx_buff, 2, 500);
  if(status == HAL_OK)
  {
    SPI1_CS_Force(ENABLE);
...
HAL_StatusTypeDef ASK25_25AA160A_Write_Byte (uint16_t eep_address, uint8_t byte_data)
{
  ...
  SPI1_CS_Force(DISABLE);
  status=HAL_SPI_Transmit(&hspi1_25aa160a, Tx_Buf, 4, 500);
  if(status == HAL_OK)
  {
    SPI1_CS_Force(ENABLE);
    HAL_Delay(5);     // Delay for 5ms Write Cycle
...
HAL_StatusTypeDef ASK25_25AA160A_Write (uint16_t eep_address, uint8_t *data_start, uint16_t length)
{
  ...
  if(length > ip_len)
  {
    ...
    SPI1_CS_Force(DISABLE);
    status=HAL_SPI_Transmit(&hspi1_25aa160a, Tx_Buf, 3, 500);
    status=HAL_SPI_Transmit(&hspi1_25aa160a, data_start, ip_len, 500);
    if(status == HAL_OK)
    {
      SPI1_CS_Force(ENABLE);
    }
  ...
  if(length <= ip_len)
  {
    ...
    SPI1_CS_Force(DISABLE);
    status=HAL_SPI_Transmit(&hspi1_25aa160a, Tx_Buf, 3, 500);
    status=HAL_SPI_Transmit(&hspi1_25aa160a, data_start, length, 500);
    if(status == HAL_OK)
    {
      SPI1_CS_Force(ENABLE);
      HAL_Delay(5);     // Delay for 5ms Write Cycle
...
uint8_t ASK25_25AA160A_Read_Byte (uint16_t eep_address)
{
  ...
  SPI1_CS_Force(DISABLE);
  status = HAL_SPI_Transmit(&hspi1_25aa160a, Tx_Buf, 3, 500);
  status = HAL_SPI_Receive(&hspi1_25aa160a, Rx_Buf, 1, 500);
  if(status == HAL_OK)
  {
    SPI1_CS_Force(ENABLE);
...
HAL_StatusTypeDef ASK25_25AA160A_Read (uint16_t eep_address, uint8_t *dest_addr, uint16_t length)
{
  ...
  SPI1_CS_Force(DISABLE);
  status = HAL_SPI_Transmit(&hspi1_25aa160a, Tx_Buf, 3, 500);
  status = HAL_SPI_Receive(&hspi1_25aa160a, dest_addr, length, 500);
  if(status == HAL_OK)
  {
    SPI1_CS_Force(ENABLE);
  }
